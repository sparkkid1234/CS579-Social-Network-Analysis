...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    def calculate_credit(parent_node):
        for child in children_dict[parent_node]:
            if child not in children_dict.keys():
                edge2credit[tuple(sorted([child,parent_node]))] = credits[child]/len(node2parents[child])
                credits[parent_node] += edge2credit[tuple(sorted([child,parent_node]))]
            #If this node is not a leaf node
            else:
                result = calculate_credit(child)/len(node2parents[child])
                credits[parent_node] += result
                edge2credit[tuple(sorted([child,parent_node]))] = result
                
        return credits[parent_node]
	
    edge2credit = defaultdict(float)
    credits = defaultdict(float)
    children_dict = defaultdict(list)
    for node, value in node2parents.items():
        for val in value:
            children_dict[val].append(node)
       
    
    for node in node2distances.keys():
        if node != root:
            credits[node] = 1.0
    calculate_credit(root)
    
    return dict(edge2credit)
